...
import { DateAxisAdapter, DateCell } from "src/layouts/scroll/date_axis_adapter.ts";
...
export interface VDateScrollProps {
  children: (dateCell: DateCell, axis: DateAxisAdapter) => React.ReactNode;
...
export function VDateScroll({
  ...
  return (
    <div
      ...
      className={className}
    >
      {cells.map(c=>renderCell(c, dateAxisAdapter))}
    </div>
  );
}
